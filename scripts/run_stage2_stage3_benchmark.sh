...
# ============================================================
# synthmind Stage2 + Stage3 Benchmark / Ablation Runner
#
# Function:
...
#
# Example:
#   FORCE=1 bash scripts/run_stage2_stage3_benchmark.sh /Users/wyc/synthmind cpu
#
...
#   bash scripts/run_stage2_stage3_benchmark.sh /Users/wyc/synthmind cpu
# ============================================================
...
print_section "synthmind Stage2 + Stage3 Benchmark"
...
step_stage2_cgcnn() {
  ...
  script="$(find_first_script \
    "${SCRIPTS}/04_train/stage2/train_cgcnn_stage2_synthmind_compare.py" \
...
rec_sh.write_text(
    ...
    "\n"
    f"export SYNTHMIND_STAGE2_MODEL=\"{best_stage2_model}\"\n"
    f"export SYNTHMIND_STAGE2_RUN_DIR=\"{run_dirs.get(best_stage2_model, '')}\"\n"
    f"export SYNTHMIND_STAGE2_SELECTION_METRIC=\"{best_stage2_metric}\"\n"
    f"export SYNTHMIND_STAGE2_SELECTION_SCORE=\"{'' if best_stage2_score is None else best_stage2_score}\"\n"
    "\n"
    f"export SYNTHMIND_STAGE3_TOP1_MODEL=\"{best_stage3_top1_model}\"\n"
    f"export SYNTHMIND_STAGE3_TOP1_RUN_DIR=\"{run_dirs.get(best_stage3_top1_model, '')}\"\n"
    f"export SYNTHMIND_STAGE3_TOP1_SELECTION_METRIC=\"{best_stage3_top1_metric}\"\n"
    f"export SYNTHMIND_STAGE3_TOP1_SELECTION_SCORE=\"{'' if best_stage3_top1_score is None else best_stage3_top1_score}\"\n"
    "\n"
    f"export SYNTHMIND_STAGE3_GENERATIVE_MODEL=\"{best_stage3_gen_model}\"\n"
    f"export SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR=\"{run_dirs.get(best_stage3_gen_model, '')}\"\n"
    f"export SYNTHMIND_STAGE3_GENERATIVE_SELECTION_METRIC=\"{best_stage3_gen_metric}\"\n"
    f"export SYNTHMIND_STAGE3_GENERATIVE_SELECTION_SCORE=\"{'' if best_stage3_gen_score is None else best_stage3_gen_score}\"\n"
    "\n"
    f"export SYNTHMIND_BENCHMARK_SUMMARY=\"{summary_csv}\"\n"
    f"export SYNTHMIND_RECOMMENDED_CONFIG=\"{rec_json}\"\n"
)

rec_md.write_text(
    "# synthmind Stage2/Stage3 Benchmark Recommendation\n\n"
...
echo
echo "============================================================"
echo "[DONE] synthmind Stage2 + Stage3 benchmark completed"
echo "============================================================"
