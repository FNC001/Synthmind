...
#
# Examples:
#   bash scripts/run_inference_pipeline.sh /Users/wyc/synthmind cpu
#
#   FORCE=1 \
#   CONDITIONED_X_CSV=/Users/wyc/synthmind/data/interim/infer/case_000001_batch_001_poscars_POSCAR/stage3_conditioned_x_fallback_retrieval_baseline_element_reranked.csv \
#   bash scripts/run_inference_pipeline.sh /Users/wyc/synthmind cpu
#
#   FORCE=1 CONDITIONED_X_ROOT=/Users/wyc/synthmind/data/interim/infer \
#   bash scripts/run_inference_pipeline.sh /Users/wyc/synthmind cpu
#
# Optional env vars:
#   FORCE=0|1
...
PROJECT_ROOT="${1:-/Users/wyc/synthmind}"
DEVICE="${2:-${DEVICE:-cpu}}"
FORCE="${FORCE:-0}"
...
if [[ -f "${RECOMMENDED_CONFIG_SH}" ]]; then
  ...
  export SYNTHMIND_STAGE2_MODEL="${SYNTHMIND_STAGE2_MODEL:-stage2_gflownet_element_bias}"
  export SYNTHMIND_STAGE2_RUN_DIR="${SYNTHMIND_STAGE2_RUN_DIR:-${PROJECT_ROOT}/runs/stage2/gflownet_benchmark_element_bias_mild_v1}"

  export SYNTHMIND_STAGE3_TOP1_MODEL="${SYNTHMIND_STAGE3_TOP1_MODEL:-stage3_residual_mdn}"
  export SYNTHMIND_STAGE3_TOP1_RUN_DIR="${SYNTHMIND_STAGE3_TOP1_RUN_DIR:-${PROJECT_ROOT}/runs/stage3/benchmark_residual_mdn_mixed_v1}"

  export SYNTHMIND_STAGE3_GENERATIVE_MODEL="${SYNTHMIND_STAGE3_GENERATIVE_MODEL:-stage3_residual_flow_mixed}"
  export SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR="${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR:-${PROJECT_ROOT}/runs/stage3/benchmark_residual_flow_mixed_v1}"
fi

SYNTHMIND_STAGE2_MODEL="${SYNTHMIND_STAGE2_MODEL:-stage2_gflownet_element_bias}"
SYNTHMIND_STAGE2_RUN_DIR="${SYNTHMIND_STAGE2_RUN_DIR:-${PROJECT_ROOT}/runs/stage2/gflownet_benchmark_element_bias_mild_v1}"

SYNTHMIND_STAGE3_TOP1_MODEL="${SYNTHMIND_STAGE3_TOP1_MODEL:-stage3_residual_mdn}"
SYNTHMIND_STAGE3_TOP1_RUN_DIR="${SYNTHMIND_STAGE3_TOP1_RUN_DIR:-${PROJECT_ROOT}/runs/stage3/benchmark_residual_mdn_mixed_v1}"

SYNTHMIND_STAGE3_GENERATIVE_MODEL="${SYNTHMIND_STAGE3_GENERATIVE_MODEL:-stage3_residual_flow_mixed}"
SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR="${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR:-${PROJECT_ROOT}/runs/stage3/benchmark_residual_flow_mixed_v1}"
...
resolve_flow_ckpt() {
  ...
  local candidates=(
    "${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}/best_stage3_residual_flow_mixed.pt"
    "${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}/best_stage3_condition_mixture_flow_mixed.pt"
    "${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}/best_stage3_condition_residual_flow_mixed.pt"
...
payload = {
    ...
    "stage2_model": "${SYNTHMIND_STAGE2_MODEL}",
    "stage2_run_dir": "${SYNTHMIND_STAGE2_RUN_DIR}",
    "stage3_top1_model": "${SYNTHMIND_STAGE3_TOP1_MODEL}",
    "stage3_top1_run_dir": "${SYNTHMIND_STAGE3_TOP1_RUN_DIR}",
    "stage3_generative_model": "${SYNTHMIND_STAGE3_GENERATIVE_MODEL}",
    "stage3_generative_run_dir": "${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}",
...
print_section "synthmind Inference Pipeline"
...
echo "  Stage2 model         = ${SYNTHMIND_STAGE2_MODEL}"
echo "  Stage2 run dir       = ${SYNTHMIND_STAGE2_RUN_DIR}"
echo "  Stage3 top1 model    = ${SYNTHMIND_STAGE3_TOP1_MODEL}"
echo "  Stage3 top1 run dir  = ${SYNTHMIND_STAGE3_TOP1_RUN_DIR}"
echo "  Stage3 gen model     = ${SYNTHMIND_STAGE3_GENERATIVE_MODEL}"
echo "  Stage3 gen run dir   = ${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}"
echo
...
step_preflight() {
  ...
  check_dir "${SYNTHMIND_STAGE2_RUN_DIR}"
  check_dir "${SYNTHMIND_STAGE3_TOP1_RUN_DIR}"
  check_dir "${SYNTHMIND_STAGE3_GENERATIVE_RUN_DIR}"
...
echo
echo "============================================================"
echo "[DONE] synthmind inference pipeline completed"
echo "============================================================"
