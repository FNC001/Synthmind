...
def main() -> None:
    ap = argparse.ArgumentParser(description="Write synthmind paper Results/Methods/Figure caption drafts.")
...
synthmind: Structure-Guided Precursor and Synthesis-Route Prediction for Inorganic Materials

## Abstract draft

Predicting inorganic synthesis routes remains difficult because precursor choices, reaction conditions, and route-level ranking are tightly coupled. synthmind addresses this task from a structure or composition input using a staged pipeline: Stage2 generates chemistry-checked precursor sets, Stage3 proposes distributional synthesis conditions, and Stage35 ranks complete precursor-condition routes.
...
**Figure 1.** synthmind pipeline from structure/composition input through Stage2 precursor generation, chemistry checking, Stage3 condition generation, and Stage35 route ranking.
