...
def regression(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    return {
...
def discrete(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    return {
...
def predict(mod: Any, model_pack: Mapping[str, Any], input_dir: Path, refined_dir: Path) -> Dict[str, Any]:
    ...
    packs = {split: mod.load_npz(input_dir / f"{split}.npz") for split in ["val", "test"]}
    ...
    for split, pack in packs.items():
        ...
        core_mask = np.asarray([m in CORE_METHODS for m in methods], dtype=bool)
...
def apply_core_val_calibration(pred_data: Dict[str, Any]) -> Dict[str, Any]:
    cont_names: Sequence[str] = pred_data["cont_names"]
    disc_names: Sequence[str] = pred_data["disc_names"]
    val = pred_data["splits"]["val"]
    calibration: Dict[str, Any] = {"continuous_bias": {}, "discrete_override": {}}
    for method in sorted(CORE_METHODS):
        msel = (val["methods"] == method) & val["core_mask"]
    ...
    for split in ["val", "test"]:
        ...
        for method in sorted(CORE_METHODS):
            idx = np.where((rec["methods"] == method) & rec["core_mask"])[0]
            for j, name in enumerate(cont_names):
...
def evaluate_one(rec: Mapping[str, Any], cont_names: Sequence[str], disc_names: Sequence[str], subset: np.ndarray) -> Dict[str, Any]:
    out: Dict[str, Any] = {"n": int(subset.sum()), "continuous": {}, "discrete": {}, "condition_success": {}}
    for j, name in enumerate(cont_names):
        mask = subset & (rec["cont_mask"][:, j] > 0.5)
        if mask.sum() >= 2:
            out["continuous"][name] = regression(rec["y_cont"][mask, j], rec["pred_cont"][mask, j])
    for j, name in enumerate(disc_names):
        mask = subset & (rec["disc_mask"][:, j] > 0.5)
        if mask.sum() >= 2:
            out["discrete"][name] = discrete(rec["y_disc"][mask, j], rec["pred_disc"][mask, j])
    try:
    ...
    out["condition_success"] = {
        ...
        "temp_le_100_all_rows": float((evaluable & (temp_err <= 100.0)).sum() / denom_all),
        "time_le_24_all_rows": float((evaluable & (time_err <= 24.0)).sum() / denom_all),
        "atmosphere_correct_all_rows": float((evaluable & atm_ok).sum() / denom_all),
    }
    return out


def evaluate(pred_data: Dict[str, Any]) -> Dict[str, Any]:
    cont_names = pred_data["cont_names"]
    disc_names = pred_data["disc_names"]
    result: Dict[str, Any] = {"val": {}, "test": {}}
    for split in ["val", "test"]:
        rec = pred_data["splits"][split]
        result[split]["core"] = evaluate_one(rec, cont_names, disc_names, rec["core_mask"])
        result[split]["by_method"] = {}
        for method in sorted(CORE_METHODS):
            result[split]["by_method"][method] = evaluate_one(rec, cont_names, disc_names, (rec["methods"] == method) & rec["core_mask"])
    if "calibration" in pred_data:
        result["calibration"] = pred_data["calibration"]
    return result


def main() -> None:
    ap = argparse.ArgumentParser(description="Evaluate Stage3 model on core methods with optional core-val calibration.")
    ...
    pred_data = predict(mod, model_pack, Path(args.input_dir).resolve(), Path(args.refined_dir).resolve())
    if args.calibrate_core_val:
        pred_data = apply_core_val_calibration(pred_data)
    metrics = evaluate(pred_data)
    out_dir = Path(args.output_dir).resolve()
    write_json(out_dir / "stage3_core_eval_metrics.json", {"config": vars(args), "metrics": metrics})
