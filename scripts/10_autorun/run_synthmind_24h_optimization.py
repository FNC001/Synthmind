...
def build_package(project_root: Path, out: Path) -> Path:
    ...
    reproduce.write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        "python scripts/10_autorun/run_synthmind_24h_optimization.py --project_root . --output_root outputs/autorun/24h_optimization_20260613 --allow_update_selector 0\n",
        encoding="utf-8",
    )
    manifest = []
    ...
    tar_path = out / "synthmind_24h_results_package.tar.gz"
    if tar_path.exists():
        tar_path.unlink()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run synthmind 24h controlled optimization, evaluation, and paper artifact generation.")
    ...
    (out / "synthmind_24h_final_report.md").write_text("\n".join(final), encoding="utf-8")
