...
def build_stage2_features(r):
    ...
    r.run([
        "python",
        r.project_root / "scripts/07_infer/structure_to_synthesis_route/pipeline/src/05_build_hybrid_features_infer.py",
        "--task", "stage2",
        "--output_dir", out_dir,
...
def build_stage2_npz(r):
    ...
    r.run([
        "python",
        r.project_root / "scripts/07_infer/structure_to_synthesis_route/pipeline/src/07_build_stage2_gflownet_infer_npz.py",
...
def sample_stage2_gflownet(r):
    ...
    Supported modes:
      - standard:
          use 06_sample_stage2_gflownet_infer.py
      - composition_biased:
    ...
    if sampler_mode == "composition_biased":
        ...
        script = project_root / "scripts/07_infer/structure_to_synthesis_route/pipeline/src/19_sample_stage2_gflownet_composition_constrained.py"
        ...
        script = project_root / "scripts/07_infer/structure_to_synthesis_route/pipeline/src/06_sample_stage2_gflownet_infer.py"
...
def _route_scripts_dir(r) -> Path:
    return _project_root(r) / "scripts" / "07_infer" / "structure_to_synthesis_route"
...
def constrain_stage2_by_composition(r):
    ...
    script = route_scripts / "pipeline/src/18_constrain_stage2_sample_candidates_by_composition.py"
...
def add_composition_fallback(r):
    ...
    script = route_scripts / "pipeline/src/11_add_composition_fallback_precursors.py"
...
def retrieve_stage2_candidates(r):
    ...
    script = route_scripts / "pipeline/src/17_retrieve_stage2_precursor_candidates_from_npz.py"
...
def merge_stage2_sources(r):
    ...
    script = route_scripts / "pipeline/src/16_merge_stage2_candidate_sources.py"
...
def rerank_stage2_by_elements(r):
    ...
    script = route_scripts / "pipeline/src/10_rerank_stage2_candidates_by_elements.py"
...
def summarize_stage2(r):
    ...
    script = project_root / "scripts" / "07_infer" / "structure_to_synthesis_route/pipeline/src/08_summarize_stage2_gflownet_candidates.py"
