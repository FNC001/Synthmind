...
def apply_input_rank_limit(df: pd.DataFrame, limit: int) -> pd.DataFrame:
    if limit <= 0:
        return df
    ranks = pd.to_numeric(df["rank"], errors="coerce").fillna(10**9)
    return df[ranks <= limit].copy()
...
def main() -> int:
    ...
    parser.add_argument("--input_pool_rank_limit", type=int, default=200)
    ...
    print(f"[rsp_vnext] loading train_csv={args.train_csv}", flush=True)
    train = apply_input_rank_limit(normalize(pd.read_csv(args.train_csv)), args.input_pool_rank_limit)
    print(f"[rsp_vnext] train rows after rank limit: {len(train)}", flush=True)
    print(f"[rsp_vnext] loading val_csv={args.val_csv}", flush=True)
    val = apply_input_rank_limit(normalize(pd.read_csv(args.val_csv)), args.input_pool_rank_limit)
    print(f"[rsp_vnext] val rows after rank limit: {len(val)}", flush=True)
    ...
    results: dict[str, Any] = {
        ...
        "input_pool_rank_limit": args.input_pool_rank_limit,
    ...
    print("[rsp_vnext] finished base metrics", flush=True)

    best = {"name": "base_rsp_v5_score", "metric": results["variants"]["base_rsp_v5_score"].get("precursor_exact@1", 0.0)}
    grid_rows = []
    for wf in [-0.25, 0.0, 0.25, 0.50, 0.75, 1.00, 1.25, 1.50]:
        for wm in [0.0, 0.25, 0.50, 0.75]:
    ...
    print("[rsp_vnext] finished family-prior grid", flush=True)
    ...
    try:
        ...
        print("[rsp_vnext] finished MLP reranker", flush=True)
