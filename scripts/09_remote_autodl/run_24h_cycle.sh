...
REMOTE_ROOT="${AUTODL_REMOTE_ROOT:-/root/synthmind_autorun_20260613}"
PROJECT_DIR="${AUTODL_PROJECT_DIR:-${REMOTE_ROOT}/synthmind}"
OUTPUT_ROOT="${AUTODL_OUTPUT_ROOT:-outputs/autorun/24h_optimization_20260613}"
SESSION_NAME="${AUTODL_TMUX_SESSION:-synthmind_24h_20260613}"
...
detect_python() {
  if command -v conda >/dev/null 2>&1; then
    ...
    for p in "${base}/envs/py311/bin/python" "${base}/envs/synthmind/bin/python" "${base}/bin/python"; do
      [[ -x "${p}" ]] && { echo "${p}"; return; }
    done
  fi
