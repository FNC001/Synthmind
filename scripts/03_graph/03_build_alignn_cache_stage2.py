...
def process_split(
    ...
    for row in rows:
        ...
        alignn_payload = build_alignn_ready_payload(struct)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build ALIGNN-ready stage2 cache with train_mode-aware split selection.")
    parser.add_argument(
        "--base_dir",
        type=str,
        default="/Users/wyc/synthmind/data",
        help="Base data directory.",
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        default="/Users/wyc/synthmind/data/interim/splits/structdesc_splits",
        help="Directory containing stage2 split jsonl files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="",
        help="Optional explicit output directory. If omitted, defaults to base_dir/interim/graph_cache/alignn_stage2/<train_mode>.",
    )
    parser.add_argument(
        "--train_mode",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="/Users/wyc/synthmind/data/interim/features/structdesc_features/meta/precursor_vocab.json",
        help="Fixed precursor vocab json. If missing, falls back to building from the selected train split.",
    )
    ...
    if args.output_dir:
        ...
        output_dir = base_dir / "interim" / "graph_cache" / "alignn_stage2" / args.train_mode
