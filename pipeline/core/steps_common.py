...
def make_infer_split(r):
    ...
    r.run([
        "python",
        r.project_root / "pipeline/core/01_make_infer_split_from_poscars.py",
...
def build_structdesc(r):
    ...
    r.run([
        "python",
        r.project_root / "pipeline/core/02_build_infer_structdesc_direct.py",
...
def build_chgnet_embedding(r):
    ...
    r.run([
        "python",
        r.project_root / "pipeline/core/03_build_infer_graph_embeddings_chgnet.py",
        "--infer_jsonl", r.outputs["infer_jsonl"],
        "--work_dir", out_dir,
...
def finalize_graph_embedding(r):
    ...
    if cgcnn_cache.is_dir() and Path(graph["cgcnn_checkpoint"]).exists() and Path(graph["cgcnn_model_py"]).exists():
        r.run([
            "python",
            r.project_root / "pipeline/core/04_finalize_infer_graph_embeddings.py",
            "--project_root", r.project_root,
            "--work_dir", r.work_dir,
