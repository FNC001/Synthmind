...
def load_matsci_pca_views(
    ...
    with np.load(path, allow_pickle=False) as cache:
        ...
        val_raw = np.concatenate(
            [cache["val_query_common_mean"], cache["val_query_role_mean"]], axis=1
        ).astype(np.float32)
    ...
    val = normalized_rows(pca.transform(val_raw))
    metadata = {
        "schema": schema,
        "components": int(components),
        "explained_variance_ratio": float(pca.explained_variance_ratio_.sum()),
    }
    return label, train, val, metadata
...
def evaluate_grid(
    ...
    for alpha in alphas:
        score_rows = []
        ordered_family_rows = []
        for (unique, keys, _), (start, end) in zip(prepared_rows, spans):
            ...
            score_rows.append(blended_scores)
            by_family: Dict[Tuple[str, ...], List[int]] = defaultdict(list)
            for index, key in enumerate(keys):
                by_family[key].append(index)
            for indices in by_family.values():
                indices.sort(key=lambda index: (-float(blended_scores[index]), index))
            ordered_family_rows.append(by_family)
        for protected in protected_prefixes:
            for minimum_gain in minimum_gains:
                ranked = [
                    ...
                    for (unique, keys, index_by_candidate), scores, by_family in zip(
                        prepared_rows, score_rows, ordered_family_rows
                    )
                ]
                metrics = {
        ...
        for protected in protected_prefixes:
            for candidate_window in (20, 50, 100):
                ranked = [
                    ...
                    for (unique, _, _), scores in zip(prepared_rows, score_rows)
                ]
                metrics = {
                    f"exact_hit@{cutoff}": float(
                        np.mean(
