    ...
    for raw_path in paths:
        ...
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                ...
                if int(per_source_limit) > 0:
                    values = values[: int(per_source_limit)]
                    raw_scores = raw_scores[: int(per_source_limit)]
...
def restrict_dataset_positive_rank(dataset: Dataset, minimum: int, maximum: int) -> None:
    """Train a rescue expert on honestly generated positives in a source-rank band."""
    lower = max(1, int(minimum))
    upper = int(maximum)
    if int(minimum) <= 0 and upper <= 0:
        return
    kept = []
    for index in dataset.row_indices:
        target = dataset.targets[index]
        pool = list(dataset.candidates[index][: dataset.pool_limit])
        if target not in pool:
            continue
        rank = pool.index(target) + 1
        if rank >= lower and (upper <= 0 or rank <= upper):
            kept.append(index)
    dataset.row_indices = kept


def restrict_dataset_row_indices(dataset: Dataset, path: str) -> int:
    """Restrict a dataset to an externally audited list of global row indices."""
    if not str(path).strip():
        return 0
    payload = json.loads(Path(path).expanduser().resolve().read_text(encoding="utf-8"))
    values = payload.get("row_indices", payload) if isinstance(payload, dict) else payload
    allowed = {int(value) for value in values}
    dataset.row_indices = [index for index in dataset.row_indices if index in allowed]
    return len(allowed)


def load_audited_row_indices(path: str) -> set[int] | None:
    if not str(path).strip():
        return None
    payload = json.loads(Path(path).expanduser().resolve().read_text(encoding="utf-8"))
    values = payload.get("row_indices", payload) if isinstance(payload, dict) else payload
...
def balanced_row_weights(
    ...
    """Training-only inverse-frequency weights for formula groups and cation families."""
    indices = np.asarray(row_indices, dtype=np.int64)
    if not len(indices):
        return np.zeros(0, dtype=np.float64)
    groups = np.asarray(group_values, dtype=object)[indices].astype(str)
    families = np.asarray(family_values, dtype=object)[indices].astype(str)
    group_counts = pd.Series(groups).value_counts().to_dict()
    family_counts = pd.Series(families).value_counts().to_dict()
    weights = np.asarray(
        [
            float(group_counts[group]) ** -float(group_power)
            * float(family_counts[family]) ** -float(family_power)
...
class ListwiseSetRanker(nn.Module):
    def __init__(
        ...
        chemistry_only_label_encoder: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.chemistry_only_label_encoder = bool(chemistry_only_label_encoder)
        if self.chemistry_only_label_encoder and label_chemistry is None:
            raise ValueError("chemistry-only label encoding requires formula descriptors")
    ...
    def forward(self, x: torch.Tensor, candidate_labels: torch.Tensor, candidate_numeric: torch.Tensor) -> torch.Tensor:
        ...
        if self.label_chemistry is not None:
            chemical = self.chemical_encoder(self.label_chemistry[candidate_labels])
            embedded = chemical if self.chemistry_only_label_encoder else embedded + chemical
        if self.candidate_transformer_layers > 0:
...
def evaluate(
    ...
    for x, labels, candidate_numeric, candidate_mask, positive_index in loader:
        ...
        for k in hits:
            current_hits = (found & (positive_rank <= k)).cpu().numpy().astype(np.float32)
            hits[k] += int(current_hits.sum())
            row_hits[k].extend(current_hits.tolist())
        count += len(x)
    result = {f"exact_hit@{k}": hits[k] / max(1, count) for k in hits}
    if group_values is not None:
        row_indices = list(loader.dataset.row_indices)
        groups = np.asarray(group_values, dtype=object)[row_indices].astype(str)
        if len(groups) != count:
            raise ValueError(f"group/evaluation row mismatch: groups={len(groups)}, rows={count}")
        for k, values in row_hits.items():
            frame = pd.DataFrame({"group": groups, "hit": np.asarray(values, dtype=np.float32)})
            result[f"group_macro_exact_hit@{k}"] = float(frame.groupby("group")["hit"].mean().mean())
    return result
...
def main() -> None:
    ...
    parser.add_argument(
        "--objective", choices=("cross_entropy", "topk_boundary", "soft_rank_hybrid"),
        default="cross_entropy",
    )
    parser.add_argument("--training_topk", type=int, default=10)
    parser.add_argument("--soft_rank_temperature", type=float, default=0.2)
    parser.add_argument("--soft_rank_weight", type=float, default=0.1)
    parser.add_argument("--soft_rank_slack", type=float, default=2.0)
    ...
    parser.add_argument(
        "--chemistry_only_label_encoder", action="store_true",
        help="Remove train-ID embeddings so train-unseen precursor formulas remain zero-shot scoreable.",
    )
    ...
    parser.add_argument(
        "--train_positive_rank_min", type=int, default=0,
        help="Optional honest source-rank lower bound for a hard-case rescue expert.",
    )
    parser.add_argument(
        "--train_positive_rank_max", type=int, default=0,
        help="Optional honest source-rank upper bound; 0 leaves it unbounded.",
    )
    parser.add_argument(
        "--train_row_indices_json", default="",
        help="Optional audited JSON row-index list, for example OOF pseudo-unseen training rows.",
    )
    ...
    parser.add_argument(
        "--pretraining_validation_interval",
        type=int,
        default=0,
        help="Evaluate and checkpoint every N gold-pretraining epochs; 0 disables it.",
    )
    parser.add_argument("--seed", type=int, default=20260714)
    parser.add_argument(
        "--group_balance_power", type=float, default=0.0,
        help="Inverse formula/material-group frequency sampling exponent; 1 gives equal group mass.",
    )
    parser.add_argument(
        "--family_balance_power", type=float, default=0.0,
        help="Inverse cation-family frequency sampling exponent applied after group balancing.",
    )
    parser.add_argument(
        "--selection_metric",
        choices=("exact_hit@10", "group_macro_exact_hit@10", "last"),
        default="exact_hit@10",
        help="Validation metric used for early stopping, or 'last' for fixed-epoch final refitting.",
    )
    ...
    audited_train_indices = load_audited_row_indices(args.train_row_indices_json)
    train_candidates, train_candidate_scores = load_candidates(
        args.train_candidates,
        len(train_x),
        selected_indices=audited_train_indices,
        per_source_limit=train_pool_limit,
    )
    val_candidates, val_candidate_scores = load_candidates(
        args.val_candidates,
        len(val_x),
        per_source_limit=int(args.val_candidate_limit),
    )
    ...
    restrict_dataset_positive_rank(
        train_dataset, args.train_positive_rank_min, args.train_positive_rank_max
    )
    audited_train_row_count = restrict_dataset_row_indices(
        train_dataset, args.train_row_indices_json
    )
    ...
    train_sampler = None
    sampling_weights = np.ones(len(train_dataset), dtype=np.float64)
    if float(args.group_balance_power) > 0 or float(args.family_balance_power) > 0:
        train_meta = pd.read_csv(
            input_dir / "train_meta.csv",
            usecols=["family_group_key", "family_signature_primary"],
        )
        sampling_weights = balanced_row_weights(
            train_dataset.row_indices,
    ...
    val_group_values = pd.read_csv(
        input_dir / "val_meta.csv", usecols=["family_group_key"]
    ...
    model = ListwiseSetRanker(
        ...
        chemistry_only_label_encoder=args.chemistry_only_label_encoder,
    ).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    progress_checkpoint = run_dir / "best_listwise_ranker.inprogress.pt"

    def save_progress_checkpoint(
        state: Dict[str, torch.Tensor], epoch: int, selection_value: float | None
    ) -> None:
        """Persist each new best state so an interrupted heavy run remains usable."""
        torch.save(
            {
                ...
                "best_val_selection_metric": selection_value,
                "checkpoint_stage": "in_progress",
            },
            progress_checkpoint,
        )
    best_metric = -1.0
    ...
    if int(args.gold_contrastive_pretrain_epochs) > 0:
        ...
        for pretrain_epoch in range(1, int(args.gold_contrastive_pretrain_epochs) + 1):
            ...
            interval = int(args.pretraining_validation_interval)
            if (
                interval > 0
                and pretrain_epoch % interval == 0
                and args.selection_metric != "last"
            ):
                gold_validation = evaluate(
                    model,
                    val_loader,
                    device,
                    args.rank_bias_weight,
                    args.residual_scale,
                    val_group_values,
                )
                gold_row.update({f"val_{key}": value for key, value in gold_validation.items()})
                current_gold = float(gold_validation[args.selection_metric])
                if current_gold > best_metric:
                    best_metric = current_gold
                    best_state = copy.deepcopy(model.state_dict())
                    best_pretraining_stage = f"gold_epoch_{pretrain_epoch}"
                    save_progress_checkpoint(best_state, 0, best_metric)
            gold_pretraining_log.append(gold_row)
            print(json.dumps(gold_row), flush=True)
    if pretraining_log or gold_pretraining_log:
        initial_metrics = evaluate(
            model, val_loader, device, args.rank_bias_weight, args.residual_scale, val_group_values
        )
        ...
        if (
            args.selection_metric != "last"
            and float(initial_metrics[args.selection_metric]) > best_metric
        ):
            best_metric = float(initial_metrics[args.selection_metric])
            best_state = copy.deepcopy(model.state_dict())
            best_pretraining_stage = "post_pretraining"
            save_progress_checkpoint(best_state, 0, best_metric)
    for epoch in range(1, args.epochs + 1):
        ...
        for x, labels, candidate_numeric, candidate_mask, positive_index in train_loader:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=device.type == "cuda"):
                ...
                if args.objective == "cross_entropy":
                ...
                elif args.objective == "topk_boundary":
                    ...
                    positive_score = scores.gather(1, positive_index[:, None]).squeeze(1)
                    negative_mask = candidate_mask > 0.5
                    negative_mask.scatter_(1, positive_index[:, None], False)
                    pairwise = (
                        scores - positive_score[:, None]
                    ) / max(float(args.soft_rank_temperature), 1e-4)
                    soft_rank = 1.0 + (
                        torch.sigmoid(pairwise) * negative_mask.to(pairwise.dtype)
                    ).sum(dim=1)
                    topk_penalty = F.softplus(
                        (soft_rank - float(args.training_topk))
                        / max(float(args.soft_rank_slack), 1e-4)
                    ).mean()
                    loss = F.cross_entropy(scores, positive_index) + float(args.soft_rank_weight) * topk_penalty
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 2.0)
            optimizer.step()
        ...
        current = float(epoch) if args.selection_metric == "last" else float(
            val_metrics[args.selection_metric]
        )
        row = {"epoch": epoch, "train_loss": float(np.mean(losses)), **val_metrics}
        logs.append(row)
        print(json.dumps(row), flush=True)
        if args.selection_metric == "last" or current > best_metric:
            best_metric = current
            best_epoch = epoch
            best_state = copy.deepcopy(model.state_dict())
            best_pretraining_stage = ""
            save_progress_checkpoint(
                best_state,
                best_epoch,
                None if args.selection_metric == "last" else best_metric,
            )
            bad_epochs = 0
        else:
            bad_epochs += 1
    ...
    checkpoint = {
        ...
        "best_pretraining_stage": best_pretraining_stage,
        "selection_metric": args.selection_metric,
        "best_val_selection_metric": None if args.selection_metric == "last" else best_metric,
    }
    torch.save(checkpoint, run_dir / "best_listwise_ranker.pt")
    progress_checkpoint.unlink(missing_ok=True)
    summary = {
        "config": vars(args),
        "data": {
            ...
            "train_positive_rank_min": int(args.train_positive_rank_min),
            "train_positive_rank_max": int(args.train_positive_rank_max),
            "train_row_indices_json": str(args.train_row_indices_json),
            "audited_train_row_count": int(audited_train_row_count),
            "group_balance_power": float(args.group_balance_power),
            "family_balance_power": float(args.family_balance_power),
            "sampling_weight_min": float(sampling_weights.min()),
            "sampling_weight_max": float(sampling_weights.max()),
