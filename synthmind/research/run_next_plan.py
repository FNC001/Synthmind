...
from synthmind.research.candidate_pool import stable_file_hash
from synthmind.research.metrics.registry import MetricRegistry
from synthmind.research.splits import choose_group_key, fingerprint_rows
...
def final_summary(project_root: Path, branch: str) -> None:
    write_text(
        project_root / "reports/synthmind_next_research_summary_v1.md",
        "\n".join([
            "# synthmind Next Research Summary v1",
            "",
            "## Direct Answers",
...
def write_manifest(project_root: Path) -> None:
    files = [
        ...
        "reports/synthmind_next_research_summary_v1.md",
    ]
    rows = []
    for rel in files:
        p = project_root / rel
