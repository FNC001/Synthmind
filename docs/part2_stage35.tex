...
\textbf{模型}:\code{sklearn.ensemble.ExtraTreesClassifier}(joblib 序列化)。synthmind 配置:
\begin{lstlisting}[language=Python]
...
\begin{enumerate}
    ...
    \item 不需要标准化也能训(但 synthmind 仍标准化,因 \code{x\_struct} 与 \code{precursor\_y} 量纲差异大)。
...
V21 是 synthmind 实测\textbf{最强单点排序器}。优势:
\begin{itemize}
...
这一 reward \textbf{不是 ground truth}——synthmind 没有"实验合成成功率"的标签,是用一组化学家可解释的启发式规则模拟"哪些路线更可能成功"。这是个\textbf{弱监督}信号。
\end{repobox}
...
三档标签 (1.0 / 0.5 / 0.0) 给的是分类信号;\code{v3\_rank\_target} 给的是连续 listwise 信号。synthmind 默认用前者。
...
synthmind 在"无真实标签 + 化学领域知识 + 多模型集成"三点上做了独特取舍。把 chemistry 知识(template / element coverage / warnings)显式注入 reward 而非让 ranker 端到端学,牺牲了一些抽象表达力,换来\textbf{可解释、可审计、可手动修正}——这对合成路线推荐场景至关重要。
