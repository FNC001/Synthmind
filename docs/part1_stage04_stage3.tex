...
到 Stage 03 (Data) 结束,synthmind 已经把 \texttt{stage3\_v5\_mixed} NPZ 准备好,每个 NPZ 含:
...
\param{--baseline\_ckpt} 是必填参数。synthmind 推荐先用 \file{train\_baseline\_linear.py} 训一份 Ridge baseline pickle,再以此 baseline 训 Flow。如果 baseline ckpt 是 torch 形式,需要给 \param{--baseline\_hidden\_dims "512,256"} 还原结构。
\end{repobox}
...
若你想最大利用每条样本(逐维 mask),改用 \file{train\_condition\_residual\_mdn\_mixed.py:masked\_nll}(\S\ref{sec:mdn-residual}),它把 dim 维 mask 加权进 \code{log\_prob\_dim},允许部分维有标签即参与训练。这两种处理在 synthmind 数据上 MAE 差异 \textasciitilde 3\%,Flow 版本更稳但 MDN 版本利用率更高。
\end{repobox}
...
即\textbf{按 $1/\text{freq}$ 加权}。开关 \param{--use\_class\_weights} 控制。synthmind 默认 ON,因为 atmosphere 的类别极不均衡(\code{air\_or\_oxidizing} 占 \textasciitilde 70\%)。
...
\code{oracle\_best\_of\_k} = "如果让 oracle 在 K 个采样里挑最好的,误差是多少"——这是上限性能,反映分布质量而非 top-1 accuracy。synthmind 默认 \param{--n\_gen\_samples 8}。
...
工程上,Mixture Flow 是"美的",但 LGBM 是"稳的"。synthmind 实测:LGBM 在 MAE 上比 Flow 略好(\textasciitilde 5\%),且训练时间从 30min 降到 3min,无需 GPU。配置里 \param{primary\_model=lgbm} 是默认。
...
\textbf{关键工程指标}:\code{top1\_within\_50} = "预测温度误差 $\le$ 50\,°C 的比例",这比 MAE 更直接反映"工程上可用"。synthmind 实测:LGBM \code{top1\_within\_100} \textasciitilde 75\%,Flow top1 \textasciitilde 70\%。
...
\begin{repobox}[可重复要点 \#4 — synthmind 推荐的 baseline 链]
\begin{enumerate}
